sap.ui.define([
	...
	return BaseController.extend("com.evonik.appsDocumentManager.controller.App", {
		...
		handleUploadPress: function(oEvent) {
			...
			oFileUploader.setUploadUrl("/docservice/DocumentManager/DirectoryList/" + this.getRepositoryName());
		...
		handleUploadComplete: function(oEvent) {
			...
			if (sStatus && sStatus === 200) {
				...
				sap.m.MessageToast.show(this.getResourceBundle().getText("msgFileUploadSuccess"));
				...
				sap.m.MessageToast.show(this.getResourceBundle().getText("msgFileUploadFail"));
			}
			this._oFileUploadDialog.close();
		},
		handleDelete: function(oEvent) {
			...
			var dialog = new Dialog({
				...
				type: 'Message',
				content: new Text({
					text: this.getResourceBundle().getText("txtConfirmDeleteFile")
				}),
				beginButton: new Button({
					text: this.getResourceBundle().getText("deleteDialogPositive"),
					press: function() {
						...
						$.ajax({
							...
							success: function(result) {
								...
								this.getView().getModel().refresh(true);
								sap.m.MessageToast.show(this.getResourceBundle().getText("msgDeleteFileSuccess"));
							}.bind(this),
							error: function(error) {
								sap.m.MessageToast.show(this.getResourceBundle().getText("msgDeleteFileFail") + error);
							}
						});
						dialog.close();
					}.bind(this)
				...
				endButton: new Button({
					text: this.getResourceBundle().getText("deleteDialogNegative"),
					press: function() {
						dialog.close();
					}
				}),
